...
Base.metadata.bind = engine # bind the engine to the metadata of the Base class so that the declaratives can be accessed


DBSession = sessionmaker(bind=engine)


# DBSession instance established represents a staging zone for all objected loaded into the database session
# It won't be persisted into the database until you call session.commit()
# You can also rollback to the last commit by calling session.rollback()
# 
